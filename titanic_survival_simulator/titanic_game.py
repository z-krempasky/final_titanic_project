...
def getIntro(name, gender, age, position, cabin, children, parents, sibling, crew_pos):
    ...
    if position == "Crew":
        positionString = "\n \nAs a member of the "+ crew_pos +  " crew on the Titanic, you are a little worried about this journey." \
                         "\nYou but as you reach the boarding site for crew members, you realize that your worries are unfounded." \
...
def submit_persona():
    ...
    cabin = cabin_var.get()
    crew_position = crew_pos_var.get()
    ...
    intro = getIntro(name, gender, age, passenger_crew, cabin, children, parents, siblings, crew_position)
...
# Create main window
root = tk.Tk()
root.title("Persona Information")
...
tk.Label(root, text="If Crew, Position").grid(row=5, column=0, sticky="w")
...
passenger_crew_radio1 = tk.Radiobutton(root, text="Passenger", variable=passenger_crew_var, value="Passenger")
passenger_crew_radio2 = tk.Radiobutton(root, text="Crew", variable=passenger_crew_var, value="Crew")
passenger_crew_radio1.grid(row=3, column=1, sticky="w")
passenger_crew_radio2.grid(row=3, column=2, sticky="e")

cabin_var = tk.StringVar()
cabin_var.set("First Class")  # Default value
cabin_menu = tk.OptionMenu(root, cabin_var, "First Class", "Business Class", "Economy Class")
cabin_menu.grid(row=4, column=1)

crew_pos_var = tk.StringVar()
crew_pos_var.set("Crew Position")  # Default value
crew_pos_menu = tk.OptionMenu(root, crew_pos_var, "Engineering", "Victualling", "Restaurant", "Deck")
crew_pos_menu.grid(row=5, column=1)
